...
BarWidget {
  ...
  // ---- Bar sizzle: a hairline under the label that drains with the day.
  //      It costs one repaint a minute, reads as a rule rather than a
  //      widget, and answers "how much of today is left" without a click.
  //      Off with "barProgress": false.
  readonly property bool barProgress: setting("barProgress", true) !== false
  readonly property real dayDone: Model.dayProgressAt(displayDate)
  ...
  // Month stepping over IPC as well as by wheel, chevron and keyboard, so the
  // calendar can be driven from a keybind or a script the same way it is
  // driven by hand.
  function moveMonth(delta) {
    if (panelLoader.item) panelLoader.item.moveMonth(delta)
  }

  function goToToday() {
    if (panelLoader.item) panelLoader.item.goToToday()
  }
  ...
  IpcHandler {
    ...
    function nextYear(): void { root.moveMonth(12) }
    function prevYear(): void { root.moveMonth(-12) }
  ...
  WidgetButton {
    ...
    // Horizontal bars only: a vertical stack has no run of label to draw a
    // rule under.
    Item {
      visible: root.barProgress && !root.vertical
      anchors.horizontalCenter: parent.horizontalCenter
      anchors.bottom: parent.bottom
      anchors.bottomMargin: Math.round(button.verticalPadding * 0.45)
      ...
      Rectangle {
        width: Math.round(parent.width * root.dayDone)
